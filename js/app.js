...
function renderizarCard(dadosCard) {
  ...
  botaoExcluir.addEventListener("click", function () {
    ...
    arrayDeCards = arrayDeCards.filter(
      (c) => c.matricula !== dadosCard.matricula,
    );
  ...
  botaoEditar.addEventListener("click", function () {
    ...
    document.querySelector("#nome-edicao").value = dadosCard.nome;
    document.querySelector("#cargo-edicao").value = dadosCard.cargo;

    document.querySelector(`.sexo-edicao[value="${dadosCard.sexo}"]`).checked =
      true;
    ...
    formEdicao.onsubmit = function (e) {
      ...
      const novoSexo = document.querySelector('input[name="genero-edicao"]:checked',).value;

      dadosCard.nome = document.querySelector("#nome-edicao").value;
      dadosCard.cargo = document.querySelector("#cargo-edicao").value;
      dadosCard.sexo = novoSexo;

      let arrayDeCards = carregarCards();
      arrayDeCards = arrayDeCards.map((c) =>
        c.matricula === dadosCard.matricula ? dadosCard : c,
      );
      salvarCards(arrayDeCards);

      divCard.remove();
      renderizarCard(dadosCard);
